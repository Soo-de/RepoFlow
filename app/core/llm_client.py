...
class LLMClient:
    """Multi-provider async LLM client supporting Gemini and Groq with automatic fallback."""
    ...
    async def generate(self, prompt: str) -> str:
        """Generate response string from Gemini, Groq, or automatic fallback."""
        ...
        # Auto mode: Try Gemini first, fall back to Groq on missing key or 429 rate limit
        if self._gemini_key:
            try:
                logger.info("Attempting pipeline generation via Gemini API...")
                return await self._generate_gemini(prompt)
            except RuntimeError as err:
                if self._groq_key and ("rate limit" in str(err).lower() or "429" in str(err)):
        ...
        raise RuntimeError(
            "No valid LLM API key provided. Please configure GEMINI_API_KEY or GROQ_API_KEY in your .env file."
        )
